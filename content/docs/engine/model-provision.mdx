...
order, hashes stable bytes, rejects symlinks, hard-linked files, and special
objects, detects tree changes during the scan, and writes a content-addressed
receipt outside the model tree.
It prints:
...
```json
{
  ...
  "files": [{ "path": "config.json", "size": 123, "sha256": "<sha256>" }]
}
```
...
| Add a cache, lock file, or downloader sidecar                   | New content digest; keep such state outside the root |
...
Source: [`optima/model_provision.py`](https://github.com/latent-to/optima/blob/main/optima/model_provision.py).
