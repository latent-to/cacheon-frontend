...
<Callout type="error" title="Release acceptance is conjunctive">
  ...
  refusal, not a warning that another artifact can override. The dated [State of record](/docs/reference/state-of-record) reports which gates the pinned
...
rejects dangerous environment keys. Release authorization must prove that the
effective SGLang argument vector cannot override validator-owned model, TP,
loader, network, or management policy through exact flags, aliases,
abbreviations, duplicates, or configuration indirection. Environment keys
require the same closed treatment. Exact-name filtering alone is
insufficient.
...
<Callout type="warn" title="Operational proof requirement">
  ...
  topology. Completed evidence is tracked in [State of record](/docs/reference/state-of-record).
</Callout>
