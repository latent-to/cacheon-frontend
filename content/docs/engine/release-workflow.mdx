...
<Callout type="error" title="Release acceptance is conjunctive">
  A deployable release requires every gate on this page: clean installation and runtime import,
  signed native-profile propagation, closed effective SGLang arguments and management routes,
  builder-output-to-registry binding, externally anchored signatures, exact host policy, and
  release/session-bound execution receipts. A missing gate is a refusal, not a warning that another
  artifact can override. The dated [State of record](/docs/reference/state-of-record) reports which
  gates the pinned implementation and retained evidence actually satisfy.
</Callout>
...
<Callout type="warn" title="Operational proof requirement">
  Unit and slice coverage of the typed release, registry, authorization, runtime, and receipt gates
  does not authorize production rollout. Release evidence must include clean installation from the
  emitted wheel, provider-specific native reopening, authenticated builder outputs, closed effective
  serving policy, a reproducible registry image pair, and descriptor/session-bound serving evidence
  at the approved topology. Completed evidence is tracked in [State of
  record](/docs/reference/state-of-record).
</Callout>
...
- Install the emitted wheel in a clean environment and import/exercise every
  runtime entrypoint reachable from the signed manifest; source-tree imports
  are not this proof.
- For each [sealed direct artifact](/docs/architecture/direct-artifacts), retain the
  provider-specific prebuild, compile-profile, index, observed-ABI, and
  publication reopen products before converting them into the generic native
  release identity.
- Prove that effective SGLang arguments, environment, loader, network, and
  management routes equal the signed policy after parsing — not merely that a
  short denylist is absent.
- Bind each no-cache builder's emitted output to the registry object admitted
  into the reproducibility statement.
...
- Bind every serve receipt to the release descriptor and one smoke-session
  identity. For sealed direct artifacts, require per-rank AOT load and
  invocation coverage in addition to ordinary seam coverage.
