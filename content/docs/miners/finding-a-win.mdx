...
controls for diagnosis, but authoritative qualification binds all four arms
from the validator's stack and reference manifests.
...
5. Run a matched graph-on whole-engine diagnostic using the
   [canonical local profiling record](/docs/validators/running-evals#performance-development)
