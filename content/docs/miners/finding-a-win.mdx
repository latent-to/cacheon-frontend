...
does not measure a component delta. The local `evaluate` CLI exposes such flags
for diagnosis, but authoritative qualification binds all four arms from the
validator's stack and reference manifests.
...
5. Run graph-on `evaluate` with matched controls and inspect B/C/B′ drift.
6. Run the relevant `bench`/quality diagnostics.
