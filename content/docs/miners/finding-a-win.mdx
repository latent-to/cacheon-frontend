...
it to [Discovery](/docs/miners/discovery-lane).
...
The developer `evaluate --input-len` option can generate longer inputs, but
repeated timed prompts with radix caching enabled may turn later passes into
cache hits. For a prefill diagnostic, use the arena's cache policy or explicitly
disable the radix cache in both arms. Treat this as local debugging, not
authority.
...
```text
B  = incumbent before candidate
C  = same stack with exactly the selected target delta
B' = incumbent after candidate
T  = separately defined pristine quality reference
```

Changing a backend, graph flag, prompt mix, topology, or engine option only in C
does not measure a component delta. Authoritative qualification binds all four
arms from the validator's stack and reference manifests.

The repository implementations are
[`stack_plan.py`](https://github.com/latent-to/optima/blob/main/optima/stack_plan.py)
for incumbent/candidate stack construction and
[`scoring.py`](https://github.com/latent-to/optima/blob/main/optima/eval/scoring.py)
...
This forces the optimization story to connect all the way from a measured bottleneck to
a selectable variant. “My kernel is 30% faster” is incomplete until you can state what
fraction of which arena path it replaces and how often the domain routes.
...
4. Trace the complete incumbent and candidate developer launches by reading
   the qualification runner and stack-plan construction, since there is no
   local command that reproduces them.
5. Repeat across fresh processes, seeds, and conditioning until the effect is
   larger than normal variance.

Steps 1–5 are miner-side diagnostics. Only finalized, isolated, identity-bound,
...
The [MiniMax-M3 case study](/docs/validators/minimax-m3-case-study) is useful
historical engineering context for how profiling led to fused collective
boundaries. Its earlier measurements do not substitute for qualification under
the current target, stack, evidence, and two-pass authority.

Once the mechanism survives this scrutiny, prepare the source-only archive and
follow [Submitting](/docs/miners/submitting).
