...
[qualification_intake.py](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_intake.py).
...
`evaluate --disable-cuda-graph` exists for debugging startup or math. It changes
the incumbent execution regime and therefore cannot establish a production
speedup. A graph-on candidate must be compared with the graph-on incumbent under
the same evaluation stack.

When graph verification fails, use the stage-specific guidance in
[Diagnostics](/docs/miners/diagnostics). Do not work around it by narrowing metadata until
