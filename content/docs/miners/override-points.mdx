...
There is currently one implemented override-point identity:
...
[override.py](https://github.com/latent-to/optima/blob/main/optima_kernels/override.py).

The status matters:

- manifest parsing, point lookup, composition, and the dense/CPU reference path
  are implemented and tested;
- the GPU `nvfp4_moe_megakernel.run` path is currently a stub that raises
  `NotImplementedError`;
- therefore the committed example is useful for ABI development but cannot
  establish a GPU crown through this base today.

See the explicit status in
[nvfp4_megakernel.py](https://github.com/latent-to/optima/blob/main/optima_kernels/moe/nvfp4_megakernel.py).
Do not quote provenance measurements embedded in example metadata as results of
the current override bundle.
...
`DSL_JIT_ENTRYPOINTS` table contains only CuTe today. The exception exists only
...
The current admission logic is pinned in
[`optima/dsl_jit_policy.py`](https://github.com/latent-to/optima/blob/main/optima/dsl_jit_policy.py)
and [`optima/engine_tree.py`](https://github.com/latent-to/optima/blob/main/optima/engine_tree.py).

The exact device ABI belongs to the registered point. For the current point,
...
Study the committed
[swigluoai override example](https://github.com/latent-to/optima/tree/main/examples/miner_m3_swigluoai_override),
...
Use a [dependency patch](/docs/miners/dep-patches) only when a registered target explicitly
requires an approved dependency export/build change. Use the
[Discovery lane](/docs/miners/discovery-lane) when the desired hole, base, or engine change
