...
[manifest.py](https://github.com/latent-to/optima/blob/main/optima/manifest.py).
...
[capabilities.py](https://github.com/latent-to/optima/blob/main/optima/capabilities.py)
and routing is in
[registry.py](https://github.com/latent-to/optima/blob/main/optima/registry.py).
...
```json
{
  "steps": [{ "type": "repo_python", "path": "build_cute_cubin.py" }]
}
```
...
If the algorithm needs intermediate storage, declare `[[ops.artifact_resources]]`. `workspace.*` is call-local, `prepared.*` crosses
...
Use the field tables in [Bundle manifest](/docs/reference/manifest-schema) and the
runtime model in [Sealed direct artifacts](/docs/architecture/direct-artifacts).
...
the target catalog. See [Override points](/docs/miners/override-points) and
...
Next, read the [Kernel ABI](/docs/miners/kernel-abi), then build the minimal example in
