...
[submit.py](https://github.com/latent-to/optima/blob/main/optima/chain/submit.py),
and the canonical payload is defined by
[payload.py](https://github.com/latent-to/optima/blob/main/optima/chain/payload.py).
...
The repository's
[submission terms](https://github.com/latent-to/optima/blob/main/docs/SUBMISSION_TERMS.md)
...
`chain-package` or the bundle hash on that extracted root. The wrapper directory and gzip
encoding are not the identity; the sorted relative paths and file bytes are. Never
“refresh” a stable URL with revised content after committing the old hash.
...
Current transport bounds include a 64 MiB archive, 256 MiB extracted content,
4,096 members, and a 60-second transfer deadline. The validator accepts only
regular identity-bearing archive members and re-hashes the safely extracted
tree. See
[fetch.py](https://github.com/latent-to/optima/blob/main/optima/chain/fetch.py).
...
5. A promoted candidate receives a complete isolated B/C/B′/T qualification
   attempt.
...
5. `promoted` means all five non-crown screens passed and capacity may now be spent on
   B/C/B′/T. It carries no speed score and no reward.
...
## Do not use the legacy local ledger as production

The top-level `commit`, `reveal`, `ledger`, and `legacy-settle` commands operate
the development JSON simulation. They are useful for tests and demonstrations,
but they do not create finalized chain intake, production qualification,
transactional settlement, or weights.

If the proposal stalls or fails, map its reported lifecycle state through
[Diagnostics](/docs/miners/diagnostics).
