...
The local JSON commit/reveal/ledger simulator and its `legacy-settle`
companion have been removed from the source tree. There is no local command
that creates finalized chain intake, production qualification, transactional
settlement, or weights; only `chain-submit`, `chain-validate`, and
`set-weights` do.
