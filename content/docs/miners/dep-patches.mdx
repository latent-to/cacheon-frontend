...
The only patchable dependency is currently `flashinfer`. The policy permits
files matching:
...
The target catalog currently adds the FlashInfer patch features only to the
deep `collective.moe_finalize_ar_rmsnorm` singleton target and the
...
[dep_policy.py](https://github.com/latent-to/optima/blob/main/optima/dep_policy.py),
and target feature policy is in
[target_catalog.py](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py).
...
See [deppatch.py](https://github.com/latent-to/optima/blob/main/optima/deppatch.py).
...
```json
{
  "steps": [{ "type": "repo_python", "path": "apply_dep_patch.py" }]
}
```
...
Those are the only registered rebuild patchers. The parser rejects
...
[rebuild.py](https://github.com/latent-to/optima/blob/main/optima/rebuild.py).
...
[apply_dep_patch.py](https://github.com/latent-to/optima/blob/main/optima/patchers/apply_dep_patch.py).
...
The committed
[atomic stack fixture](https://github.com/latent-to/optima/tree/main/tests/fixtures/stack_fused_epilogue_atomic)
...
current deep ABI. Treat it as a schema fixture, not a miner kernel template.

If the desired edit is outside this closed policy, submit it through the
[Discovery lane](/docs/miners/discovery-lane) rather than disguising it as a dependency
patch.
