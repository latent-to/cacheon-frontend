...
The target catalog grants the FlashInfer patch features only to the
...
direct-artifact provider; it is not a dependency-patch installer. The parser rejects `bundle_python`, arbitrary repository scripts, duplicate patchers, unknown
