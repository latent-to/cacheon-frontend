...
| ABI                     | `optima-op-abi-v0` callable rows                                | `optima-discovery-abi-v1` closed patch manifest                          |
...
A discovery proposal is **not** an op bundle and must not use legacy
`[competition] mode = "system"`. Its closed manifest uses
...
[discovery.py](https://github.com/latent-to/optima/blob/main/optima/discovery.py).
...
policy, resource controls, B/C/B′ timing, and pristine T quality authority.
Discovery code does not get to choose its own evaluator or report its own score.
...
settlement, but not for a standing target crown.

When enabled by the emissions manifest, settlement may create one
non-renewable, expiring discovery bounty claim. The operator controls the
discovery pool, lifetime, and build-profile availability. Re-submitting or
promoting the idea does not renew the same proposal's bounty.

If review later registers a singleton or atomic target, subsequent competitive
implementations use that new target's component ABI and standing reward family.
The discovery proposal itself does not silently become the crown.

See
[economics.py](https://github.com/latent-to/optima/blob/main/optima/economics.py)
for bounty claims and
[PRODUCT_CONTRACT.md](https://github.com/latent-to/optima/blob/main/docs/PRODUCT_CONTRACT.md)
...
[Submitting](/docs/miners/submitting):
