...
You are free to choose algorithms, tiling, fusion _inside_ the slot, and honest
...
[slots.py](https://github.com/latent-to/optima/blob/main/optima/slots.py), with
output shape/stride checks in
[tensor_spec.py](https://github.com/latent-to/optima/blob/main/optima/tensor_spec.py).
...
See [Sealed direct artifacts](/docs/architecture/direct-artifacts) and the
[manifest field reference](/docs/reference/manifest-schema).
...
by target and [dependency-patch](/docs/miners/dep-patches) policy.
...
replay writes will fail that stage. Continue with [Graph evidence](/docs/miners/graph-safety).
