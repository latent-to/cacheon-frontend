---
title: Your first component bundle
---
...
[GPU setup guide](/docs/dev/gpu-setup) explains the current development
...
The committed
[example bundle](https://github.com/latent-to/optima/tree/main/examples/miner_silu_torch)
...
writes, and compares the result with the trusted reference. It should end in a
`PASS` for applicable shapes.
...
Then use the engine-facing developer diagnostics while iterating:

```bash
python -m optima.cli evaluate my_silu \
  --model <arena-model> --tp-size <arena-tp> ...

python -m optima.cli bench my_silu \
  --model <arena-model> --tp-size <arena-tp> \
  --benchmarks <published-benchmark-set> ...
```

`evaluate` and `bench` are **developer diagnostics, not crown authority**. Their
flags are intentionally useful for debugging, including candidate-only launch
changes. An authoritative candidate cannot choose a different evaluation stack;
the validator binds B, C, B′, T, resources, graph evidence, hidden inputs, and
calibrated policies itself.

Never call a graphs-off result a win. `--disable-cuda-graph` is a debugging
mode that changes the incumbent regime. For a long-prefill target, also make the
workload genuinely prefill-heavy; repeated prompts with a live radix cache can
silently turn later iterations into decode/cache-hit measurements.
...
Continue with [Finding a win](/docs/miners/finding-a-win), [Graph evidence](/docs/miners/graph-safety),
and [Submitting](/docs/miners/submitting).
