...
There is no local command that loads the candidate inside the full engine and
measures throughput or fidelity; `scan` and `verify` are the complete local
contribution loop. **Neither one is crown authority.** An authoritative
candidate cannot choose a different evaluation stack; the validator binds B,
C, B′, T, resources, graph evidence, hidden inputs, and calibrated policies
itself at the registered arena boundary.

Never call a graphs-off `verify` result a win. `--disable-cuda-graph` is a
debugging mode that changes the incumbent regime.
