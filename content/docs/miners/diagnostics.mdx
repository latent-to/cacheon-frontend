...
There is no local command that reproduces or contests a retained validator
grade; `scan` and `verify` are the complete local contribution loop, and
neither one measures throughput or fidelity.
