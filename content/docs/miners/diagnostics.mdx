---
title: Diagnostics by lifecycle stage
---
...
| `qualifying` | one authority-bound B/C/B′/T attempt is active |
...
| `expired` | terminal queue-age expiry |

These transitions are implemented in
[intake.py](https://github.com/latent-to/optima/blob/main/optima/chain/intake.py).
...
- **wrong positional signature:** compare with [Kernel ABI](/docs/miners/kernel-abi);
...
Do not “fix” graph failure by using `evaluate --disable-cuda-graph`; that changes
the serving regime. See [Graph evidence](/docs/miners/graph-safety).
...
- the archive contains one valid wrapper/root and only regular identity-bearing
  files.
...
[publication.py](https://github.com/latent-to/optima/blob/main/optima/chain/publication.py).
...
Qualification aggregates mandatory graph, marginal speed, and pristine quality
evidence. Any FAIL makes the attempt fail; any `NO_DECISION` prevents PASS.
...
- pristine T or raw evidence cannot reopen: authority/infrastructure failure;
- a local `bench` pass differs: local benchmark flags and prompts are not the
  bound reference policy.

`evaluate` and `bench` explicitly print diagnostic, non-authoritative results.
They are for reproducing mechanisms, not contesting a retained validator grade.
...
Weight publication is a separate reconciled action. A crown's standing credit
depends on the active emissions policy and decays with age; there is no fixed
per-slot payout.
