---
title: Slots and contribution targets
---
...
[slots.py](https://github.com/latent-to/optima/blob/main/optima/slots.py) and
...
width of the score-sheet overlap comparison (the registered width is eight blocks),
but does not configure production selection or gate whether the score kernel
routes. A runtime call may therefore use a different top-k while the candidate
still fills the complete score sheet and the stock tail applies the engine's
value.
...
See [Kernel ABI](/docs/miners/kernel-abi) for tensor semantics and
[Graph evidence](/docs/miners/graph-safety) for capture requirements.
...
combination belongs in the [Discovery lane](/docs/miners/discovery-lane).
...
proposal. Continue with [Bundle format](/docs/miners/bundle-format).
