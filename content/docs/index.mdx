...
| Check what is implemented, measured, and still unproven | [Current status](/docs/reference/state-of-record) |

## Evidence is scoped, not blended

Cacheon has historical B300 performance wins and a completed hardened authority
implementation. Those are different evidence classes. The historical wins were measured
through the earlier referee; the hardened path has structural RTX evidence but still
requires current-path B300 qualification and a real release-image build-and-serve proof.
The [state of record](/docs/reference/state-of-record) keeps those claims separate.

<Callout type="info" title="Source of executable truth">
The [Cacheon source repository](https://github.com/latent-to/optima) owns executable
contracts, schemas, policies, and evidence verifiers. Content-addressed production
evidence and immutable publications live in separate operator-owned stores. This
site owns published explanations, tutorials, and operator guidance. When prose and
code disagree, the pinned code and its tests take precedence.
</Callout>
