...
A passing test, an empirical qualification, and a deployable release are different
evidence classes, and Cacheon does not let a strong result in one class stand in for
another. The strongest retained performance authority is a real B200 blockscore
qualification with independently reproduced PASSes and a settled crown; the checked-out
branch is separately not release-ready, since clean-wheel import, provider-specific
native proof, and closed serving-argument authorization remain open. The
[state of record](/docs/reference/state-of-record) keeps those claims separate.
