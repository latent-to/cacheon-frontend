...
[target catalog](/docs/reference/target-catalog).
...
A clean example currently looks like this:
...
summary and one row per exercised shape. Read the fields this way:
...
[diagnostics guide](/docs/miners/diagnostics) maps the same reasoning to GPU, graph,
...
evidence stores. There is no local command that substitutes for them; `scan` and `verify`
remain the complete local contribution loop.

## Continue

| If you want to…                     | Read…                                               |
| ----------------------------------- | --------------------------------------------------- |
| write your first bundle             | [Your first kernel](/docs/miners/your-first-kernel) |
| prepare a GPU development machine   | [GPU setup](/docs/dev/gpu-setup)                    |
| understand production qualification | [Qualification](/docs/validators/qualification)     |
| operate finalized intake first      | [Deployment readiness](/docs/validators/first-hour) |
