...
Cacheon Engine is the chain-independent distribution consumed by serving deployments. It
contains a pinned upstream runtime, reviewed Cacheon source, exact model identity, native
artifacts, a serving specification, provenance, an SBOM, and a signature rooted in an
external Ed25519 public key.

It does not read miner URLs, wallets, chain state, or the live evaluation incumbent.
...
5. Qualification runs the incumbent before the candidate (**B**), the same stack with
   exactly this selected target delta (**C**), and the incumbent again (**B′**). A
   separately defined pristine reference (**T**) supplies candidate-free quality
   evidence. B/C/B′ are timed; T is not a speed competitor.
...
- A **discovery proposal** is a bounded cross-cutting prototype that cannot claim a
  normal target. It may be promoted into a target, integrated as a reviewed engine
  change, or receive a one-time bounded bounty.
...
: The serving stack. Every entry is a reviewed integrated contribution.
...
Cacheon exposes useful developer commands that do not create crowns:

- `scan` checks static policy without loading a bundle; and
- `verify` compares a slot implementation with its trusted reference.

There is no local command that grants qualification or settlement authority.
...
See [Proposal to release](/docs/architecture/pipeline) for the complete sequence.
...
- [Run the CPU quickstart](/docs/get-started/quickstart)
- [Understand the architecture](/docs/architecture/overview)
- [Choose a registered target](/docs/miners/slots)
- [Inspect the current evidence boundary](/docs/reference/state-of-record)
