...
The release implementation is unit- and slice-tested, but the retained closure did
**not** run an actual joined no-cache double OCI build followed by TP8 serving and
receipt verification. The retained crown used for release-structure proof was a
synthetic 100-ppm fixture, not an empirical RTX/B300 win. No production arena provider
ships in the repository. Treat the host and serving sequence above as required future
operational validation, not a completed performance claim.
</Callout>
