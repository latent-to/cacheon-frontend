...
The current runtime distribution is `optima-engine` version `0.0.1`. Its deterministic
artifacts are:
...
The reviewed programmatic host path can:
...
7. require active, routed, and completed receipts for every released slot and rank, with
   no load-failure or fallback receipts.
...
Use a two-person or otherwise independently reviewed release procedure:
...
2. **Prepare twice-derived evidence.** Run programmatic release preparation and verify
   source/wheel reproducibility, SBOM/provenance closure, integrated-only manifest, native
   tree binding, and model receipt.
...
5. **Build and publish twice.** The reviewed host API performs two no-cache builds with no
   build network, pushes both, and requires the same registry manifest digest before
...
6. **Authorize the exact container.** Bind registry repository/digest, local image
   ...
   device request, command, and environment.
...
8. **Gate rollout on receipts.** Require active, routed, and completed receipts for every
   released slot and expected rank, with no load failure or fallback receipt. Retain the
   exact image, authorization, launch, and receipt products.
...
The repository currently provides public CLI commands to provision a model, verify an
...
<Callout type="warn" title="Current validation boundary (2026-07-13)">
  The release implementation is unit- and slice-tested, but the retained closure did **not** run an
  actual joined no-cache double OCI build followed by TP8 serving and receipt verification. The
  retained crown used for release-structure proof was a synthetic 100-ppm fixture, not an empirical
  RTX/B300 win. No production arena provider ships in the repository. Treat the host and serving
  sequence above as required future operational validation, not a completed performance claim.
</Callout>
