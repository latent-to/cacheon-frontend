---
title: A validator's first hour
---
...
package is absent or a chokepoint is broken. It is only the static seam canary.
Before treating the installed runtime as an arena-qualified pin, follow the
empirical gates and dated status linked from
[SGLang compatibility](/docs/dev/sglang-tracking).
...
On a configured GPU development host, use [Running diagnostics](/docs/validators/running-evals) to
...
4. five ordered non-crown screen timeouts;
5. provider implementation digest and qualification-policy digest; and
6. calibration, pristine reference, evidence-root, entropy, hidden-judge, OCI executor,
   and absolute-deadline authorities used by `build_qualification`.
...
Do not substitute a shell evaluator, dynamic import path, `optima evaluate`, or a fake
provider that declares success. A commissioned provider must return real typed screen
evidence and construct the real isolated B/C/B′/T qualification work.
...
This repository's retained closure proof used structural qualification fixtures; it did
not supply a deployable production provider or a current empirical GPU crown. Your
commissioning run must not relabel those fixtures as production performance evidence.

## 9. Commission the signer separately

Only after the store contains a genuine current-schema crown and complete active arena
state should the signer run `optima set-weights --dry-run`. Use the exact emissions-policy
arguments approved by the validator set; the CLI intentionally has no defaults for them.
...
Do not call the validator commissioned merely because unit and slice tests pass. As of
the 2026-07-13 state of record, the actual joined OCI double-build and TP8 serve smoke had
not run, and no production `ArenaServiceProvider` shipped with the repository.
...
- [Package metadata](https://github.com/latent-to/optima/blob/main/pyproject.toml)
- [CLI parser](https://github.com/latent-to/optima/blob/main/optima/cli.py)
- [Compatibility canary](https://github.com/latent-to/optima/blob/main/optima/compat.py)
- [Chain SDK canary](https://github.com/latent-to/optima/blob/main/optima/chain_canary.py)
