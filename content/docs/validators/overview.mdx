...
`optima verify`, `optima evaluate`, and `optima bench` are developer diagnostics.
They are useful before submission and during integration, but their output is not a
crown, a settlement record, or weight authority. See [Running diagnostics](/docs/validators/running-evals).

## Durable state

Production referee state lives in `FinalizedIntakeStore`, not the legacy JSON ledger.
The store binds its database to a chain genesis hash and netuid, records finalized
...
The JSON `commit`, `reveal`, `ledger`, and `legacy-settle --development-only` commands
remain local simulation tools. They do not describe production economics.
