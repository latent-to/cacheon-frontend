---
title: The finalized chain loop
---
...
8. **Screen and qualify.** If a registered arena service was injected, run its staged
   screens, form an admitted cohort, execute qualification, and persist outcomes.
...
The current command has no `--wallet`, `--hotkey`, `--ledger`, `--eval-cmd`,
`--margin`, or `--dry-run-weights` options. Those flags belonged to an earlier
prototype and must not appear in service units or runbooks.
...
The default `IntakePolicy` currently sets:
...
| Explicit expiry age | 2,880 blocks | Enables reviewed expiry; it is not automatic garbage collection |
...
currently wraps reservation-hold release, so deployment tooling must expose it under its
...
Continue with [Arena service](/docs/validators/arena-service) and
[Settlement and weights](/docs/validators/settlement-and-weights).

## Source anchors

- [Finalized reveal reader](https://github.com/latent-to/optima/blob/main/optima/chain/__init__.py)
- [Payload schema](https://github.com/latent-to/optima/blob/main/optima/chain/payload.py)
- [HTTPS fetcher](https://github.com/latent-to/optima/blob/main/optima/chain/fetch.py)
- [Finalized intake store](https://github.com/latent-to/optima/blob/main/optima/chain/intake.py)
- [Immutable publication](https://github.com/latent-to/optima/blob/main/optima/chain/publication.py)
- [Validator loop](https://github.com/latent-to/optima/blob/main/optima/chain/validator_loop.py)
