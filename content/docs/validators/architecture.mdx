---
title: Architecture
description: CPU validator loop, GPU eval entrypoint, S3 handoff, challenger detection, leader comparison, weight setting, state persistence, and observability.
---

## Validator main loop

The CPU validator loop lives in `validator/cpu_validator.py`. Operators usually run it through Docker Compose `validator/docker-compose.yml`, using the image built from `validator/Dockerfile.cpu`. Alternatively you can also run it directly on the host with `python -m validator.cpu_validator` while using the same environment variables and mounts.

Between ticks, the process sleeps for `poll_interval`, configured through `CACHEON_POLL_INTERVAL_S` or `--poll-interval`. The bundled Compose setup defaults to `600` seconds when that variable is unset.

```py
while True:
    s3_download(state_dir)
    state = load_state()

    metagraph, block, block_hash = scan_chain()
    commitments = parse_commitments(metagraph)
    commitments = gate_unpaid_commitments(state, commitments)  # verify payment_tx on-chain

    if winner_hotkey_deregistered():
        promote_runner_up_or_clear()

    if new_eval_results() or weights_stale():
        set_weights(leader, runner_up)

    challengers = select_unevaluated(commitments)
    if challengers:
        write_eval_job(challengers, block, block_hash)
        s3_upload(state_dir)

        if auto_rent_enabled:
            gpu = search_providers(targon, lium)
            pod = rent(gpu)
            ssh_exec(pod, setup-gpu.sh)
            ssh_exec(pod, docker compose -f gpu-compose.yml up)
            teardown(pod)
            s3_download(state_dir)  # pick up eval results

    sleep(poll_interval)
```

Weight setting triggers in three situations:

- **New eval results**: GPU completed evaluations (detected by comparing `evaluation_block` against `last_weights_set_block`)
- **First weight set**: a winner exists but weights have never been set
- **Staleness refresh**: weights are older than one tempo (~360 blocks, ~72 min) to stay active in consensus

## GPU eval entrypoint

The GPU host runs `validator/gpu_eval.py` as a one-shot job, usually from the image built by `validator/Dockerfile.gpu` and started with `validator/gpu-compose.yml`. Docker Compose mounts the host Docker socket so the runner can start baseline and miner containers for each challenger (`evaluate_in_docker` in the pseudocode below). This component has no Bittensor dependency.

```py
s3_download(state_dir)
eval_job = load_eval_job()        # written by CPU; includes current leader and runner_up
prompts = sample_prompts(block_hash)
baseline = run_baseline_if_needed(prompts)

# Leader and runner-up are re-evaluated first, then challengers.
# All scores in a round are fresh; no cached score carries forward.
for entry in eval_job.incumbents + eval_job.challengers:
    result = evaluate_in_docker(entry, prompts, baseline)
    state.record_evaluation(result)

state.rerank_round()   # pick winner and runner-up from this round's scores
s3_upload(state_dir)
exit
```

The GPU reads `eval_job.json` (block context + incumbent and challenger list), runs evaluations, calls `rerank_round` to select the winner and runner-up from fresh scores, writes results into `state.json`, and exits. Each entry is saved individually so partial results survive pod crashes.

## State and S3 handoff

The validator and GPU pod share a single S3 prefix (`validator/sync.py`). Both sides download the full state directory before starting work and upload after completing it. The validator writes `eval_job.json` and `state.json` (after weight setting); the GPU writes `state.json` (eval results, leader changes), `baseline_cache/`, `container_logs/`, and `logs/`.

`state.json` tracks the winner record, runner-up, evaluation history with per-prompt breakdowns, precheck failures (including unpaid submissions), and scan/weight-setting block numbers. Written atomically (temp file + rename).

Typical mirrored layout under `state-mainnet/`:

```
state-mainnet/
  state.json
  eval_job.json
  winner-history.jsonl
  baseline_cache/
    baseline_{hash}.json
  container_logs/
    baseline_{hash}.log
    uid{N}_{hotkey}_{block}.log
  logs/
    gpu_eval_{timestamp}.log
```

JSON field-level detail: [Data contracts](/docs/evaluation/data-contracts).

## Observability

Each GPU eval run captures:

- **Per-prompt metrics**: TTFT, throughput, token match rate, output token count for every scored prompt
- **Container logs**: full stdout/stderr from baseline and miner containers, saved to `container_logs/`
- **Timestamped validator logs**: one log file per GPU eval session under `logs/`

All artifacts are uploaded to S3 and available for the frontend monitoring API.
