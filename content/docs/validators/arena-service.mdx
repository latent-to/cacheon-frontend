...
A hold is durable and never disappears merely because time passed. The store exposes two
explicit reservation-level dispositions: `release_hold`, with an audited reason, and
`expire`, which is admitted only after the configured minimum age. It does not expose an
automatic TTL or a typed transition that retires an entire arena. Settlement lease expiry
and a discovery claim's configured lifetime are different state machines and do not clear
an arena hold. Operators must monitor held rows and preserve their evidence; deleting or
silently aging them out can reorder authority and is not a supported recovery path.
...
`run_screen` must return the requested stage and typed evidence. The service always passes
...
| `executor`                    | Runs untrusted complete-engine roles under the reviewed OCI authority                                   |
...
3. Run an isolated qualification control that demonstrates B/C/B′/T ordering, cleanup,
   raw-evidence reopen, and `PASS`/`FAIL`/`NO_DECISION` separation.
...
Next: [Authoritative qualification](/docs/validators/qualification).

## Source anchors

- [Arena service types and registry](https://github.com/latent-to/optima/blob/main/optima/arena_service.py)
- [Arena service tests](https://github.com/latent-to/optima/blob/main/tests/test_arena_service.py)
- [Qualification intake projection](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_intake.py)
