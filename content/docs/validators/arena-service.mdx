...
- `hold` when queue depth, queue age, cohort size, or retry policy has crossed a bound.
...
`run_screen` must return the requested stage and typed evidence. `build_qualification`
must preserve the exact promoted reservation order and qualification-policy digest while
supplying a plan factory, isolated executor, post-commit entropy provider, hidden judge,
...
- The retained repository proof used test/structural providers. No production provider
  for a real evaluator fleet exists in the repository as of the 2026-07-13 state of
  record.
