...
controller. It does not come from `optima evaluate`, an in-engine self-audit, a miner
report, or an arbitrary evaluator command.
...
- candidate ordering is derived from post-commit entropy;
- the selection receipt covers the exact chosen prompts/tasks and ordering;
...
For registered cohorts, an infrastructure failure before a complete attempt produces
`NO_DECISION` for every affected reservation and a persisted bisection plan. Subsequent
passes halve the cohort to isolate a poisoning or resource-sensitive candidate in
logarithmic retry groups. A per-candidate `NO_DECISION` after a complete shared attempt is
requeued individually. Neither mechanism changes finalized arrival order.
...
evidence roots, confirms both dispositions still carry PASS authority, and binds a new
settlement-evidence receipt. The slower passing speedup is used even if the primary was
faster.

## Reopen and regrade

The final report is derived from retained raw artifacts and calibration manifests. The
system can reopen those products, verify their hashes and identities, and rederive graph,
speed, and quality decisions. A summary JSON line without its evidence is not authority.

See [Evidence and replay](/docs/security/evidence) for retention and audit requirements.

An authoritative attempt is not one file. The retained closure includes the authority
manifest, selected plan and commitment/entropy receipts, B/C/B′ lifecycle and device
receipts, graph or discovery-execution evidence, raw charged-speed batches, pristine T
session exchanges, raw quality products, per-candidate reports, and the enclosing attempt
artifact. Settlement keeps references to both complete attempt roots.

Reopening verifies hashes and expected bindings before grading. If the attempt artifact
reopens but a referenced raw product does not, authority is still incomplete. Operators
must retain the complete evidence graph and test restores, not merely archive the final
report digest.
...
| Worker, Docker, GPU, driver, or evidence publication failure | `NO_DECISION`; repair infrastructure and use bounded retry |
...
- The current repository does not ship the production provider that constructs this work
  for a real arena. Its retained structural two-PASS closure fixture is not an empirical
  GPU crown or a current production calibration.
