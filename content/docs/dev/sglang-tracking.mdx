...
At the current snapshot, the static seam canary is green for `0.5.13.post1`,
while the GPU end-to-end broken-bundle rejection, faithful-bundle pass, and
champion rebaseline are still pending. `0.5.12.post1` remains the last pin with
complete retained H100 validation. Treat the source pin as the compatibility
target, not as proof that those empirical gates have already run.
...
or an old H100 champion from being quoted as a current baseline.
...
See [Current status](/docs/reference/state-of-record) for which empirical GPU
