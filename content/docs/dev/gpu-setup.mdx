...
There is no local command that loads a candidate inside the full engine and
measures throughput or fidelity; `scan` and `verify` are the complete local
contribution loop. Move upward only after `verify` is green:
...
an injected arena service; no local command-line result substitutes for that
path. Do not copy a `verify` result into the current state of record as if it
came from the hardened qualification path.
