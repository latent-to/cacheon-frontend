...
This page prepares a contributor or maintainer GPU host for component
verification and performance development. It is not a production arena
...
The canonical [performance-development procedure](/docs/validators/running-evals#performance-development) defines the required inputs and local result record; no repository command materializes
this complete-engine bracket.

Production qualification constructs complete B/C/B′ engines and a candidate-free
pristine T session through an injected arena service. A contributor-controlled model run
