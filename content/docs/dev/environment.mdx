...
5. **Arena qualification** for complete-engine B/C/B′ timing and candidate-free
   T quality. Only this layer can create qualification authority.
...
[GPU setup](/docs/dev/gpu-setup). Run `python -m optima.cli compat`, then explicitly
compare its printed installed version with `PINNED_SGLANG`; a mismatch is
reported but does not currently make the canary exit nonzero.
...
See the [codebase map](/docs/reference/codebase-map) for subsystem ownership.
...
Source: [`pyproject.toml`](https://github.com/latent-to/optima/blob/main/pyproject.toml).
