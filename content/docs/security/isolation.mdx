...
entry code. `evaluate` and `bench` load candidate code in model workers and can request a
diagnostic no-egress network namespace. These are development properties, not a substitute
...
- Scheduler-only candidate load gate: post-main working-tree path `optima/integrations/sglang_scheduler_gate.py`
