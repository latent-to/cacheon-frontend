...
sequenceDiagram
    ...
    participant R as GPU runtime OCI
    ...
    H->>R: Exact tree/native/model + role/workload
    R-->>H: B/C/B′ bounded frames + sealed trajectory
    H->>H: Destroy candidate and prove cleanup
    H->>T: Candidate-free reference request
    T-->>H: Untimed quality evidence
    H->>H: Regrade and persist verdict
```

There is no direct P→R artifact handoff that bypasses the host's publication, inventory,
and reopen checks. There is no C→T process reuse: candidate destruction precedes the
pristine quality lifetime.
...
mount. Candidate files are parsed as data, and only validator-registered rebuild patchers
may run in the build phase. The host never imports candidate Python or loads the produced
native object.
...
Source scanning remains useful defense in depth. It rejects known-dangerous syntax and
recursively covers declared source, CUDA sources, patches, and vendored tree content.
But scanning cannot prove arbitrary code safe, especially generated native code.

The current scanner admits the required CuTe DSL form `cute.compile(...)` only
...
`verify` uses fresh spawned workers so the CLI process does not import normal candidate
entry code. It is a development property, not a substitute for the production OCI,
mount, device, evidence, and cleanup authority. The direct engine-facing evaluator and
benchmark commands that previously loaded candidate code in model workers with an
optional diagnostic no-egress namespace have been removed from the source tree.
...
See [Threat model](/docs/security/threat-model) and [Evidence and replay](/docs/security/evidence).

## Source anchors

- [Engine launch identities](https://github.com/latent-to/optima/blob/main/optima/eval/engine_launch.py)
- [OCI prebuild](https://github.com/latent-to/optima/blob/main/optima/eval/oci_prebuild.py)
- [OCI process leases](https://github.com/latent-to/optima/blob/main/optima/eval/oci_process.py)
- [OCI runtime backend](https://github.com/latent-to/optima/blob/main/optima/eval/oci_backend.py)
- [Reference session](https://github.com/latent-to/optima/blob/main/optima/eval/oci_reference_session.py)
- [Scheduler-only candidate load gate](https://github.com/latent-to/optima/blob/main/optima/integrations/sglang_scheduler_gate.py)
