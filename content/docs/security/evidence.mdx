...
`reopen_causal_qualification(..., expected=...)` requires a reconstructed `CausalQualificationInput`. The expected object supplies the full prepared plan, candidate
authorities, graph evidence references and requirements, calibration artifact/reference
context, runtime resource policy, T/reference authority, and commitment. The persisted `QualificationAuthorityManifest` carries identity fields and commitments, not this full
...
| Execution          | Durable `SpeedWitness` with three aggregate B/C/B′ charged-rate rows; richer session frames and per-arm device/cleanup observations are validated in-run but are not serialized into the attempt             |
...
A full evidence regrade is available only when the exact expected `CausalQualificationInput` can be reconstructed. It proceeds from identities, not from a
desired verdict:
...
4. Validate the three aggregate `ChargedExecutionRate` rows, recompute their tokens/second
   values from retained counts and intervals, and regrade speed under frozen calibration.
...
### Sampled seam audit is not universal authority

[`optima/audit.py`](https://github.com/latent-to/cacheon/blob/main/optima/audit.py) can emit sampled comparison receipts for supported live dispatch seams when its runtime
flags are explicitly armed. The isolated qualification worker currently clears those
flags, and qualification does not consume sampled audit receipts as a required input.
They are therefore optional diagnostics, not a universal cross-slot correctness gate or
settlement authority.

Required authority still comes from the registered verifier and graph checks, canonical
execution receipts, B/C/B′ measurement, and candidate-free T policy. The absence of a
sampled audit receipt must not be rewritten as either a pass or a failure unless a future
typed policy explicitly binds, retains, and grades that evidence.
...
1. reopen each report's `SpeedWitness` and verify its B/C/B′ launch and distinct session
   identities;
2. verify retained conditioning/timed/charged token counts, intervals, sums, and the witness
   projection digest;
3. recompute the three aggregate rates, B/B′ drift, and candidate speed decision under the
   frozen calibration;
...
- Optional sampled in-engine audit receipts are not a universal qualification or
  settlement authority.
