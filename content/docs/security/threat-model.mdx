...
`verify`, `evaluate`, and `bench` are diagnostics. Their spawned processes and optional
network namespace are not equivalent to the production OCI controller, evidence chain,
or pristine reference. `--allow-unsafe-no-isolation` is explicitly unsafe and must never
be used for crownable work.
