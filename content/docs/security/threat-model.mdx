...
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Hostile URL, SSRF, redirect, or archive         | HTTPS only; TLS 1.2 minimum; globally routable DNS answers; pinned reviewed IP with SNI/hostname verification; every redirect revalidated; strict archive/member/size/path rules; committed hash rederived | CA/DNS/origin compromise can affect availability; network stack and TLS library remain trusted                                    |
...
| Candidate tampers with timer or role            | Host assigns B/C/B′ roles, owns clocks, validates bounded raw batches/token counts, observes device state, and controls teardown                                                                           | Host clocks, firmware, driver, and provider scheduling must be trustworthy and calibrated                                         |
| Candidate fakes quality                         | C is destroyed before candidate-free pristine T teacher-forces sealed trajectories; candidate and B′ are not grading oracles                                                                               | Reference bugs and finite hidden-work coverage remain possible                                                                    |
...
| Candidate exploits noise                        | B/C/B′ bracketing, frozen calibration, maximum-noise threshold, `NO_DECISION`, independent reproduction, lower reproduced speedup                                                                          | Hardware drift and cross-validator variance remain operational concerns                                                           |
...
and no network. See [Isolation](/docs/security/isolation).
...
`set-weights` runs separately against durable settlement state and live chain readback.
...
`verify` is a diagnostic. There is no local command that grants qualification or
settlement authority; the direct evaluator and benchmark commands have been removed
from the source tree. `verify`'s spawned processes are not equivalent to the production
OCI controller, evidence chain, or pristine reference. `--allow-unsafe-no-isolation` is
explicitly unsafe and must never be used for crownable work.
...
- [HTTPS intake](https://github.com/latent-to/optima/blob/main/optima/chain/fetch.py)
- [OCI native prebuild](https://github.com/latent-to/optima/blob/main/optima/eval/oci_prebuild.py)
- [OCI runtime controller](https://github.com/latent-to/optima/blob/main/optima/eval/oci_backend.py)
- [Causal qualification](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_runner.py)
- [Weight publication](https://github.com/latent-to/optima/blob/main/optima/chain/weights.py)
- [Release host](https://github.com/latent-to/optima/blob/main/optima/release_host.py)
