...
The local `verify` command is a developer diagnostic. It does not mint production crown authority, and there is no local command that reproduces qualification or settlement. See [Evaluation pipeline](/docs/architecture/pipeline) for the detailed state machine and [Releases](/docs/architecture/releases) for the ship boundary.
