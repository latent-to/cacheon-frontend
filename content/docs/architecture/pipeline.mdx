...
| --------------- | ---------------------------------------------------------------------------------------------------- | ---------------------- |
| `slots`         | Inspect the registered slot ABI                                                                      | Informational          |
| `scan`          | Run recursive static bundle policy checks                                                            | Diagnostic             |
...
flowchart TD
    ...
    S --> T["Transactional settlement and stack update"]
...
Principal code: [`chain/intake.py`](https://github.com/latent-to/optima/blob/main/optima/chain/intake.py) and [`chain/validator_loop.py`](https://github.com/latent-to/optima/blob/main/optima/chain/validator_loop.py).
...
Principal code: [`chain/fetch.py`](https://github.com/latent-to/optima/blob/main/optima/chain/fetch.py), [`chain/payload.py`](https://github.com/latent-to/optima/blob/main/optima/chain/payload.py), and [`bundle_hash.py`](https://github.com/latent-to/optima/blob/main/optima/bundle_hash.py).
...
Principal code: [`arena_service.py`](https://github.com/latent-to/optima/blob/main/optima/arena_service.py), [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py), and [`stack_plan.py`](https://github.com/latent-to/optima/blob/main/optima/stack_plan.py).
...
Principal code: [`eval/qualification_intake.py`](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_intake.py) and [`stack_plan.py`](https://github.com/latent-to/optima/blob/main/optima/stack_plan.py).
...
A candidate may fill its target through a [sealed direct artifact](/docs/architecture/direct-artifacts) instead of ordinary Python: a validator-owned prebuild compiles it to CUBIN before the timed arm starts, and qualification additionally requires `aot_loaded`, `aot_invoked`, and `completed` receipts with no fallback before the run can count.

Principal code: [`eval/qualification.py`](https://github.com/latent-to/optima/blob/main/optima/eval/qualification.py), [`eval/qualification_runner.py`](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_runner.py), and [`eval/oci_backend.py`](https://github.com/latent-to/optima/blob/main/optima/eval/oci_backend.py).
...
5. Settlement reopens both evidence roots, takes the lower accepted speedup, revalidates
   the live target transition, and atomically updates the evaluation stack.
6. Reward projection can now see the active claim. Product integration remains a separate
   review; no proposal bytes have entered a release merely because settlement completed.
...
Settlement reopens both evidence roots instead of trusting an in-memory verdict. It verifies:

- both reports are complete `PASS` results;
- authority and evidence roots are distinct;
...
The conservative settled speedup is the lower accepted speedup from the two passes. The stack transition and settlement evidence are committed transactionally; a partial write cannot expose a half-updated incumbent.

Principal code: [`settlement.py`](https://github.com/latent-to/optima/blob/main/optima/settlement.py).
...
Principal code: [`economics.py`](https://github.com/latent-to/optima/blob/main/optima/economics.py), [`chain/weights.py`](https://github.com/latent-to/optima/blob/main/optima/chain/weights.py), and the [emissions policy](https://github.com/latent-to/optima/blob/main/docs/EMISSIONS_POLICY.md).
...
See [Release architecture](/docs/architecture/releases). Current implementation and validation limits are tracked in [State of record](/docs/reference/state-of-record).
...
- registered arena, target catalog, incumbent manifest, candidate transition, and screen
  receipt;
- B/C/B′ launch identities, raw bounded session evidence, device/cleanup receipts, and
  the candidate-free T evidence;
- frozen calibration and the exact policy that maps the raw evidence to the verdict;
- distinct primary and reproduction authorities over one reproduction identity;
- transactional settlement events and resulting evaluation-stack digest;
- reward projection, publication intent, chain submission/readback, and confirmed or held
  state; and
...
not a substitute for the corresponding reopenable product. There is no local command that
grants qualification or settlement authority; the older local JSON commit/reveal/ledger
simulator and its `legacy-settle` companion have been removed from the source tree, and
production operator flow uses only `chain-validate` and `set-weights`.
