...
```text
slots -> scan -> verify -> evaluate / bench -> chain-package -> host -> chain-submit -> chain-status
```
...
| `evaluate` | Run a narrow model-level development comparison | Diagnostic |
| `bench` | Run benchmark-oriented throughput and quality diagnostics | Diagnostic |
...
## Legacy simulation is not production authority

`commit`, `reveal`, `ledger`, and `legacy-settle --development-only` exercise an older local mechanism simulator. They do not write production SQLite qualification, settlement, or weight state. Production operator flow uses `chain-validate` and `set-weights`.

The distinction matters when reading old examples or historical receipts: a successful legacy flow does not demonstrate the current isolated two-pass production path.
