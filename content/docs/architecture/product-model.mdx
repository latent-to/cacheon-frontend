...
Cacheon Engine is a chain-independent, open inference-acceleration distribution. It pins an upstream runtime, initially SGLang, and assembles accepted data-plane optimizations into a canonical engine stack.
...
The [canonical product contract](https://github.com/latent-to/optima/blob/main/docs/PRODUCT_CONTRACT.md) is the normative source for this boundary.
...
This permits emissions to follow crown policy while production deployment follows release policy. See [Release architecture](/docs/architecture/releases).

## Marginal contribution, complete execution

Cacheon deliberately separates economic identity from process identity.

The **execution unit** is a complete engine. B, C, B′, and T each run in isolated engine processes because candidate code must not share the controller's trust domain.
...
See also [the submission model](/docs/concepts/submission-model).
...
1. The validator materializes B and B′ from the exact incumbent stack containing A.
2. It materializes C from that same stack, replacing only B's declared registered target.
...
4. T grades the sealed C trajectory using a pristine candidate-free reference. A is not
   allowed to grade B merely because A is in the incumbent.
...
The evaluation stack is an economic hill-climb state. The release manifest is a product state. The reference manifest is semantic authority. A crown can transactionally update the first; only reviewed promotion and signing can create a new instance of the second. See [Stacks and manifests](/docs/architecture/stacks).
...
The catalog currently registers all singleton slots and the atomic `collective.moe_epilogue.v1` target. The atomic target owns both `collective.ar_residual_rmsnorm` and `collective.moe_finalize_ar_rmsnorm`, and explicitly displaces the corresponding singleton titles while active. The live policy is implemented in [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py).
...
- a successful prototype is either promoted into a reviewed slot or atomic boundary, integrated as a bounded contribution, or paid a bounded reviewed bounty.
...
3. The trusted controller never imports candidate code. Complete engines remain the isolation unit, and a [sealed direct artifact](/docs/architecture/direct-artifacts)'s compiler factory runs only in a further-isolated, no-network, no-GPU prebuild child — never in the controller, the timed engine, or a serving release.
...
3. **Where does hostile code execute?** Proposal code belongs only in the complete isolated
   evaluation engine, never in the controller or serving release by reference. A direct-artifact
   factory belongs only in its further-isolated no-network/no-GPU compiler child, never in the
   timed engine itself.
...
- [`docs/PRODUCT_CONTRACT.md`](https://github.com/latent-to/optima/blob/main/docs/PRODUCT_CONTRACT.md) — normative product invariants
- [`stack_manifest.py`](https://github.com/latent-to/optima/blob/main/optima/stack_manifest.py) — proposal, integration, evaluation-stack, and release-stack identities
- [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py) — reward-unit policy
- [`engine_tree.py`](https://github.com/latent-to/optima/blob/main/optima/engine_tree.py) — deterministic proposal and integrated-source materialization
- [`release.py`](https://github.com/latent-to/optima/blob/main/optima/release.py) — chain-independent release artifacts and signatures
