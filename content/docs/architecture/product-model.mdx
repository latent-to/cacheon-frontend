...
3. The trusted controller never imports candidate code; candidate runtime execution stays
   inside a complete isolated engine, while a [sealed-direct-artifact](/docs/architecture/direct-artifacts)
   factory may execute only inside its further isolated no-network/no-GPU compiler child.
