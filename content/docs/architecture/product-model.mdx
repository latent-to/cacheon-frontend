...
3. The trusted controller never imports candidate code; complete engines remain the isolation unit.
...
3. **Where does hostile code execute?** Proposal code belongs only in the complete isolated
   evaluation engine, never in the controller or serving release by reference.
