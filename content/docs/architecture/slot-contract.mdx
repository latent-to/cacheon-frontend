...
Slots may evolve, SGLang adapters may churn, and correctness policies may be recalibrated. Every core slot must still satisfy the four invariants on this page. The normative checklist is [`docs/SLOT_CONTRACT.md`](https://github.com/latent-to/optima/blob/main/docs/SLOT_CONTRACT.md); the executable catalog is [`optima/slots.py`](https://github.com/latent-to/optima/blob/main/optima/slots.py).
...
Runtime MSA `topk` controls downstream block selection, while the slot correctness
`top_k` used by `attention.msa_block_score` and `attention.msa_prefill_block_score` is
only the verifier's overlap width. The two are not the same knob: changing the verifier's
`top_k` tolerance does not change which blocks the live engine actually selects.
...
The implementation is split between [`verify_collective.py`](https://github.com/latent-to/optima/blob/main/optima/verify_collective.py), [`dispatch.py`](https://github.com/latent-to/optima/blob/main/optima/dispatch.py), and the version-pinned adapters in [`integrations/`](https://github.com/latent-to/optima/tree/main/optima/integrations).
...
See [Graph safety](/docs/miners/graph-safety) for bundle-facing guidance.
...
See [Product model](/docs/architecture/product-model) and [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py).
...
- [`docs/SLOT_CONTRACT.md`](https://github.com/latent-to/optima/blob/main/docs/SLOT_CONTRACT.md) — normative invariants
- [`slots.py`](https://github.com/latent-to/optima/blob/main/optima/slots.py) — executable slot catalog
- [`tensor_spec.py`](https://github.com/latent-to/optima/blob/main/optima/tensor_spec.py) — typed outputs and workspaces
- [`verify.py`](https://github.com/latent-to/optima/blob/main/optima/verify.py) — op/block verification and graph replay
- [`verify_collective.py`](https://github.com/latent-to/optima/blob/main/optima/verify_collective.py) — distributed verification
- [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py) — economic target projection
