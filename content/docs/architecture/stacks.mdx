...
Canonical digest construction lives in [`stack_identity.py`](https://github.com/latent-to/optima/blob/main/optima/stack_identity.py); strict manifest types live in [`stack_manifest.py`](https://github.com/latent-to/optima/blob/main/optima/stack_manifest.py).
...
| Engine-tree digest    | Which exact deterministic materialized files were emitted? | Selected source closure, namespace rewriting, rebuild plan, or emitted inventory changes |
...
A contribution that uses a [sealed direct artifact](/docs/architecture/direct-artifacts)
adds identities at every layer above rather than replacing them:

| Layer                  | What the direct artifact contributes                                                          |
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| Selected contribution    | The canonical `optima.direct-artifact-execution.v1` declaration digest                             |
| Stack manifest digest    | Included through the selected-delta digest of the target it fills                                  |
| Engine-tree digest       | The manifest's export declarations, not the compiled CUBIN bytes                                    |
| Launch identity          | The reopened native build spec, compile profile, and sealed CUBIN publication digest                |

Two trees can share a stack digest and a tree digest while still requiring separately
reopened native publications, because compilation happens after materialization in a
disposable prebuild rather than being baked into the emitted source tree. See
[Runtime selection and evidence](/docs/architecture/direct-artifacts#runtime-selection-and-evidence)
for the receipts that close that gap.
...
flowchart TB
    ...
    B["B: materialize E"]
    C["C: materialize E with exactly one target transition"]
    BP["B′: independently materialize E"]
    T["T: pristine ReferenceManifest"]

    E --> B
    E --> C
    E --> BP
    T -->|"untimed semantic grading"| C
    B -->|"host timing"| V["Qualification verdict"]
    C -->|"host timing + sealed trajectory"| V
    BP -->|"drift bookend"| V
```
...
The planner records both the old and new contribution references, the selected-delta digest, the exact target specification, and the expected execution order. See [`stack_plan.py`](https://github.com/latent-to/optima/blob/main/optima/stack_plan.py).
...
```text
B  = materialize(E0)                         # A + R0
C  = materialize(replace(E0, rmsnorm, R1))  # A + R1
B′ = materialize(E0)                         # A + R0, fresh launch
...
The operational evaluator may amortize a frozen incumbent across a chain-ordered cohort `C1..Ck`. Cohorting does not weaken marginal identity:
...
- B and B′ remain exact incumbent bookends;
- the retained evidence binds each candidate to its own selected delta;
...
This guarantees that B and B′ receive the same incumbent bindings and C receives only the binding change implied by its exact target delta. T has no candidate binding. See [SGLang seam](/docs/architecture/seam).
...
The controller prepares these inputs before timed execution. A separate
...
Principal implementations are [`eval/engine_launch.py`](https://github.com/latent-to/optima/blob/main/optima/eval/engine_launch.py), [`eval/native_artifact.py`](https://github.com/latent-to/optima/blob/main/optima/eval/native_artifact.py), and [`eval/oci_backend.py`](https://github.com/latent-to/optima/blob/main/optima/eval/oci_backend.py).
...
If validation, persistence, or readback fails, the old stack remains authoritative. Rollback is itself a planned, content-addressed transition rather than an in-place edit. See [`settlement.py`](https://github.com/latent-to/optima/blob/main/optima/settlement.py) and [`chain/intake.py`](https://github.com/latent-to/optima/blob/main/optima/chain/intake.py).
...
- [`stack_manifest.py`](https://github.com/latent-to/optima/blob/main/optima/stack_manifest.py) — strict manifest and contribution-reference types
- [`stack_plan.py`](https://github.com/latent-to/optima/blob/main/optima/stack_plan.py) — marginal arms, cohorts, transitions, and rollback
- [`engine_tree.py`](https://github.com/latent-to/optima/blob/main/optima/engine_tree.py) — deterministic source materialization and integration promotion
- [`target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py) — singleton, atomic, overlap, and composition policy
- [`eval/reference_quality.py`](https://github.com/latent-to/optima/blob/main/optima/eval/reference_quality.py) — pristine reference quality products
- [`eval/calibration.py`](https://github.com/latent-to/optima/blob/main/optima/eval/calibration.py) — calibrated qualification/reference policy
