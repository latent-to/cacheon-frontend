...
- `optima/seams.py` defines the SGLang-specific adapters that must be reviewed whenever the runtime pin moves. The current scheduler-gate row is on the post-main working branch pinned in [State of record](/docs/reference/state-of-record).
...
plus post-main working-tree paths `optima/seam.py` and
`optima/integrations/sglang_scheduler_gate.py` at the snapshot commit.
