...
- [`slots.py`](https://github.com/latent-to/optima/blob/main/optima/slots.py) defines the stable miner-facing semantic contract;
- `optima/seams.py` defines the SGLang-specific adapters that must be reviewed whenever the runtime pin moves.
...
The catalog can contain a verified slot before the pinned runtime exposes a safe live chokepoint. `attention.msa_block_score` currently has a slot and verifier contract, while [`sglang_msa.py`](https://github.com/latent-to/optima/blob/main/optima/integrations/sglang_msa.py) deliberately refuses installation until a stable decode-side MSA chokepoint is registered. The prefill sibling has a live adapter and appears in the table.
...
SGLang versions that expose a plugin framework also have an [`sglang_plugin.py`](https://github.com/latent-to/optima/blob/main/optima/integrations/sglang_plugin.py) shim. The `.pth` bootstrap remains the general spawn-safe path across the supported pin.

Principal code: [`bootstrap.py`](https://github.com/latent-to/optima/blob/main/optima/bootstrap.py),
[`seam.py`](https://github.com/latent-to/optima/blob/main/optima/seam.py), and
[`integrations/sglang_scheduler_gate.py`](https://github.com/latent-to/optima/blob/main/optima/integrations/sglang_scheduler_gate.py).
...
The dispatch implementation is [`dispatch.py`](https://github.com/latent-to/optima/blob/main/optima/dispatch.py); registration and eligibility live in [`registry.py`](https://github.com/latent-to/optima/blob/main/optima/registry.py).
...
child because architecture selection is import-time state; runtime installs only the row
for the active device. During autotuning, both the deep producer and the fused-epilogue
consumer remain stock so a candidate cannot influence tactic selection.

Principal code: [`moe_export.py`](https://github.com/latent-to/optima/blob/main/optima/moe_export.py), [`dep_policy.py`](https://github.com/latent-to/optima/blob/main/optima/dep_policy.py), and [`patchers/apply_dep_patch.py`](https://github.com/latent-to/optima/blob/main/optima/patchers/apply_dep_patch.py).
...
An adapter that fires only in eager mode, captures a cached answer, mutates storage identity, or bypasses live replay is not qualified for a graphs-on arena. See [Slot contract](/docs/architecture/slot-contract) and [Graph safety](/docs/miners/graph-safety).
...
See [`receipts.py`](https://github.com/latent-to/optima/blob/main/optima/receipts.py), [`eval/oci_session_protocol.py`](https://github.com/latent-to/optima/blob/main/optima/eval/oci_session_protocol.py), and [`eval/qualification_runner.py`](https://github.com/latent-to/optima/blob/main/optima/eval/qualification_runner.py).
...
<Callout type="warn" title="Current pin validation status">
  A green import/chokepoint canary is necessary compatibility evidence, not proof that the current
  pin preserves candidate execution and measured performance. Check [State of
  record](/docs/reference/state-of-record) for the dated claim boundary and [SGLang
  compatibility](/docs/dev/sglang-tracking) for the complete validation ladder.
</Callout>
...
- [`seams.py`](https://github.com/latent-to/optima/blob/main/optima/seams.py) — adapter and binding source of truth
- [`bootstrap.py`](https://github.com/latent-to/optima/blob/main/optima/bootstrap.py) — startup/import hook
- [`seam.py`](https://github.com/latent-to/optima/blob/main/optima/seam.py) — activation and sealed contribution loading
- [`dispatch.py`](https://github.com/latent-to/optima/blob/main/optima/dispatch.py) — typed live dispatch
- [`integrations/`](https://github.com/latent-to/optima/tree/main/optima/integrations) — version-pinned SGLang adapters
- [`compat.py`](https://github.com/latent-to/optima/blob/main/optima/compat.py) — pin and chokepoint canary
