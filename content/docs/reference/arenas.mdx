...
def build_qualification(request) -> ArenaQualificationWork: ...
```
...
| `HOLD` | Queue depth/age, cohort size, or retry budget crosses policy | Stop automatic progress pending operator/policy handling |
...
active qualification capacity is queued. This avoids converting overload into
candidate failure.
...
- Pin provider code by the digest embedded in each service manifest.
...
Source: [`optima/arena_service.py`](https://github.com/latent-to/optima/blob/main/optima/arena_service.py).
