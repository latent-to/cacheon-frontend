...
|---|---|---:|---:|---:|
| `EvaluationStackManifest` | What complete incumbent does this arena evaluate? | allowed | yes | as B/B′ |
...
- the arena digest;
- the full target-catalog snapshot and digest;
...
T is untimed. It grades sealed B/C/B′ trajectories only after candidate engines
have been destroyed. Neither the incumbent evaluation stack nor a candidate's
self-reported scores can substitute for this authority.
...
- Construct an `EvaluationStackContext` from validator-owned arena policy and
  call `validate_against()` before planning B, C, or B′.
...
Source: [`optima/stack_manifest.py`](https://github.com/latent-to/optima/blob/main/optima/stack_manifest.py) and
[`optima/eval/qualification.py`](https://github.com/latent-to/optima/blob/main/optima/eval/qualification.py).
