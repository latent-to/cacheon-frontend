...
## No local qualification path

The local JSON commit/reveal/ledger simulator, its `legacy-settle` companion,
and the direct `evaluate`/`bench` diagnostics have been removed from the
source tree. There is no local command that backs the production SQLite
controller or grants qualification authority. Start from the validator loop
and qualification runner when auditing production.
