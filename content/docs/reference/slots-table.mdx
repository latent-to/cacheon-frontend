...
<Callout type="info" title="Registered does not always mean installed">
  ...
  adapter refuses installation unless the pinned runtime exposes a stable, auditable registered
  chokepoint. Its prefill sibling has an installed adapter. See [State of
  record](/docs/reference/state-of-record) for validated coverage.
</Callout>
