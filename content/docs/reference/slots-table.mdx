...
its decode-side SGLang adapter is deliberately non-installing until the
pinned runtime exposes a stable, auditable chokepoint. Its prefill sibling
has a live adapter. See [Current status](/docs/reference/state-of-record).
</Callout>
