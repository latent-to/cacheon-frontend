...
[`optima/slots.py`](https://github.com/latent-to/optima/blob/main/optima/slots.py)
...
<Callout type="info" title="Registered does not always mean installed">
  `attention.msa_block_score` has a complete slot and verifier contract, but its decode-side SGLang
  adapter is deliberately non-installing until the pinned runtime exposes a stable, auditable
  chokepoint. Its prefill sibling has a live adapter. See [Current
  status](/docs/reference/state-of-record).
</Callout>
...
2. graph replay checks that dynamic inputs are refreshed and outputs are
   actually rewritten;
3. complete-engine B/C/B′ runs measure the exact marginal substitution;
4. pristine T grades sealed trajectories without candidate code present; and
5. fidelity/resource evidence checks that the candidate did not obtain speed
...
for work the baseline performs. `attention.decode` currently has an
eager gather form; a paged-direct graph-safe form is the intended stronger
boundary.
...
expert targets. See [Target catalog](/docs/reference/target-catalog).

For the invariant waist behind every slot, read
[Target and slot contract](/docs/architecture/slot-contract).
