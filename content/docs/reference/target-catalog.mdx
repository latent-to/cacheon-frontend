...
prove that a serving seam is live. In particular, the
`attention.msa_block_score` decode-side integration is currently a deliberate
non-installing stub pending a stable pinned-runtime chokepoint. See
...
Source: [`optima/target_catalog.py`](https://github.com/latent-to/optima/blob/main/optima/target_catalog.py).
