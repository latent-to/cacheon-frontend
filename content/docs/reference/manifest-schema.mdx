...
A separate `[[ops.aot_exports]]` table declares one or more sealed direct-artifact
execution steps for the registered `cutlass.cute.cubin.v1` provider, and
`[[ops.artifact_resources]]` declares the validator-allocated storage such a step may
use. These fields follow their own closed schema rather than the operation-row fields
above; see [Sealed direct artifacts](/docs/architecture/direct-artifacts) for the
manifest execution model, device ABI, and storage lifecycle they admit.
