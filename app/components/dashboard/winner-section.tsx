...
import { fetchWinner, fetchWinnerHistory, type WinnerHistoryEntry } from '~/lib/api.client'
import {
  fmtScore,
  fmtImprovement,
  fmtPct,
...
export function WinnerSection() {
  const winner = usePoll(fetchWinner, 30_000)
  const history = usePoll(fetchWinnerHistory, 30_000)

  const w = winner.data?.winner
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <GlassCard className="p-6">
          ...
          {winner.loading ? (
            ...
            <p className="text-secondary/60 font-mono text-sm">No winner yet</p>
          ) : (
            <>
          ...
          {history.loading ? (
            ...
            <div className="relative space-y-0">
              ...
              {[...history.data.history].reverse().map((entry, i) => (
                <WinnerHistoryNode key={entry.block} entry={entry} isLatest={i === 0} />
              ))}
            </div>
          )}
        </GlassCard>
...
function WinnerHistoryNode({ entry, isLatest }: { entry: WinnerHistoryEntry; isLatest: boolean }) {
  return (
    <div className="relative flex gap-4 py-3 pl-6">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1">
          ...
          >
            {truncHotkey(entry.new_winner_hotkey)}
          </span>
          <CopyButton value={entry.new_winner_hotkey} />
        </div>
        <div className="text-secondary/55 mt-0.5 font-mono text-[0.65rem]">
          UID {entry.new_winner_uid}
        </div>
        ...
        <div className="text-secondary/60 mt-1 font-mono text-[0.68rem]">
          Score {fmtScore(entry.new_winner_score)}
          {entry.prev_winner_uid != null && (
            <span className="text-secondary/40">
              {' '}
              (overtook UID {entry.prev_winner_uid}, was {fmtScore(entry.prev_winner_score)})
            </span>
          )}
        </div>
      </div>
